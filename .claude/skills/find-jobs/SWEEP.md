...
The eighth item is not a check, and it runs whichever way the seven above came out.

8. **Save the posting.** Run `scripts/save_spec.mjs` with Node via Bash, against the resolved `applyUrl` where there is one and the lead's own URL otherwise, passing the applications directory, the company and the title as the pipeline's prompt names them. It fetches the page itself - through the ATS public API where one covers it - and writes the text under a record directory named on the same slug rule the record itself uses, so the spec and the record land together. Report back the `slug`, `path` and `source` it prints, as `slug`, `specPath` and `specSource`; leave `specPath` null where it fails and say what it printed in the notes. Run it for a dropped lead as readily as for a surviving one: a listing is taken down within weeks, and the record for a drop is what stops the next sweep paying to find and read the same posting again. Never write the file by hand from what was read - the point of the script is that what lands on disk is the posting rather than a re-transcription of it, and verbatim is the whole property the file exists to hold.
