...
- **The CV heading renders this role as Principal Engineer & Director.** The engineering noun leads because it is what the role does and what an engineering screen looks for: the stack, the architecture and what gets built next are decided here, and the platform is built and operated here too. A heading carrying only "Director" answers a search nobody filling an engineering role is running. "Director" stays on the same line because it is the verifiable half - Companies House carries it, the front matter holds it as the record, and the ownership is a fact about the role rather than a claim about it. "Principal" describes the grade of the work rather than a conferred one; there is no framework behind it at a company this size, which is exactly why the directorship sits beside it. The bullets keep "as sole developer" alongside, because that is the scope and it is also the strongest thing about the role - a business's platform designed, built and operated by one person. There is no technical team here and no line should imply one, which rules out "Technical Lead" and "Lead Engineer" as renderings; both would also collide with the Technical Lead title held concurrently at Talis Education. "Technical Director" says which of the two directors this is, but it names an engineering function the company does not have and reads further from the bench than the role sits. "Senior Developer" and "Solutions Architect" aim at the wrong thing in the other direction - a conferred grade with no framework behind it, and a function this role does not perform
